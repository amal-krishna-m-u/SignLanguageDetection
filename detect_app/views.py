...
def detection_view(request):
    ...
    def mediapipe_detection(image, model):
        ...
        )  # Converts back to BGR for video output
    ...
    def extract_keypoints(results):
        ...
        )  # z value represents the depth or distance of a particular landmark point in relation to the camera or reference point.
    ...
    parser = argparse.ArgumentParser(
        description="Run the SignTalk real-time sign language interpreter."
    )
    ...
    colors = [
        ...
        (117, 245, 16),
        (16, 117, 245),
    ]  # different colours for different actions

    # Detection Variables
    sequence = (
        []
    )  # concat the np frames (30 frames) and append to sequence as list to be processed by the model
    sentence = []
    predictions = []
    threshold = 0.6
    ...
    capture = cv2.VideoCapture(
        0
    )  # grab camera device default -- usually webcam -- device 0 is iphone and device 1 is macbook camera
    ...
    with mp_holistic.Holistic(
        ...
        while capture.isOpened():  # if camera is available
            ...
            if (
                len(sequence) == 30
            ):  # only run predicts when the number of frames appended is 30
                res = model.predict(np.expand_dims(sequence, axis=0))[
                    0
                ]  # we expand the dim to 'fake' a batch size dim, NxCxHxW expand in N dim
                ...
                if np.unique(predictions[-10:])[0] == np.argmax(
                    res
                ):  # grab the last 10 predictions and checking if ten predictions are similar to each other and the current prediction
                    if (
                        res[np.argmax(res)] > threshold
                    ):  # checking the highest prediction probability > threshold
                        if (
                            len(sentence) > 0
                        ):  # check if actions have already been predicted
                            if (
                                actions[np.argmax(res)] != sentence[-1]
                            ):  # to check if the current action differs with the previous sentence
            ...
            if cv2.waitKey(1) & 0xFF == ord(
                "q"
            ):  # & 0xFF operation ensures that the key code is compared correctly with the ASCII code of 'q' (ord('q')).
                break
